<template>
  <div>
    ...
    <el-table :data="employees">
      <el-table-column type="selection" fixed="left" prop="id" label="编号" />
      <el-table-column fixed="left" prop="username" label="用户名" />
      <el-table-column prop="realname" label="姓名" />
      <el-table-column prop="gender" label="性别" />
      <el-table-column width="120" prop="telephone" label="电话" />
      ...
      <el-table-column fixed="right" label="操作">
        <template v-slot="slot">
          ...
          <i class="el-icon-edit" @click.prevent="toUpdateHandler(slot.row)" />
          <i class="el-icon-delete" @click.prevent="toDeleteHandler(slot.row.id)" />
        </template>
      </el-table-column>
    </el-table>
    <!-- /表格 -->
